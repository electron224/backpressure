...
export function ConceptLab({
  ...
  function injectChaos(): void {
    ...
    const backends = [
      ...new Set(
        topologies.flatMap((topology) => topology.nodes.filter((node) => node.kind === "service" || node.kind === "rate-limiter" || node.kind === "shard-router" || node.kind === "dedup").map((node) => node.id)),
      ),
    ];
